...
def post_image_to_url(path):
    ...
    res_get = requests.get(url,verify=false)
    print(res_get)
    ...
    res_post = requests.post(url,files=files,headers=headers,data=data,proxies={"http":random.choice(proxies)},timeout=3600,verify=false)
    print(res_post)
